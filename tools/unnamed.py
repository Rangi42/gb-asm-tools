...
"""
Usage: unnamed.py [-h] [-r rootdir] [-l count] project.sym
...
if args.rootdir:
	for line in subprocess.Popen(['make', '-C', args.rootdir, '-s', '-p', 'DEBUG=1'],
		...
		if (m := re.match(r'^(\w+_obj\s*:=)', line)):
			objects = [f'{args.rootdir}/{obj}' for obj in line.removeprefix(m.group(1)).strip().split()]
			break
	else:
...
print(f'Unnamed symbols: {len(symbols)} ({unnamed_percent:.2f}% complete)')
if not objects:
	for sym in symbols:
		print(sym)
	sys.exit()
